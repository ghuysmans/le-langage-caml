...
and analyse_prog = parser
  ...
  | [< 'Symbole '.' >] -> []
  | [< 'lxm >] -> raise (Stream.Error "")

and analyse_phrase = parser
  | [< 'Mot "pour"; 'Mot s; variables = paramètres; ordres = suite_d'ordres >] ->
...
and ordre = parser
  ...
  | [< 'Mot f; exprs = liste_d'expressions >] -> Exécute (f, exprs)
...
and liste_d'expressions = parser
  | [< exp = expression; l = liste_d'expressions >] -> exp::l
  | [<>] -> [];;
