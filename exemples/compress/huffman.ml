...
let encode entrée sortie codage =
  ...
  try
    while true do
      ...
      List.iter (Esbit.écrire_bit sortie)
              codage.caractère.(Char.code c)
    done
  with End_of_file ->           (* fin du fichier d'entrée *)
    List.iter (Esbit.écrire_bit sortie) codage.fin;
    Esbit.finir sortie;;
  ...
  for c = 0 to 255 do
    if fréquences.(c) > 0 then begin
      prio := Fileprio.ajoute !prio
                fréquences.(c) (Lettre(Char.chr c));
...
let arbre_vers_codage arbre =
  ...
  | Lettre c ->
      codage.caractère.(Char.code c) <- List.rev préfixe
  | Fin ->
      codage.fin <- List.rev préfixe
  | Noeud(arbre1, arbre2) ->
