...
let grep_sur_canal auto nom_fich canal =
  try
    while true do
      ...
      if Fastrec.reconnaît auto ligne then begin
        ligne_trouvée := true;
...
let construire_auto expr =
  déterminise(expr_vers_automate(lire(Caml__csl.stream_of_string expr)));;
...
if Caml__csl.interactive then () else
  if Array.length Sys.argv < 2 then begin
    ...
    let auto =
      try construire_auto Sys.argv.(1)
    ...
    if Array.length Sys.argv >= 3 then
      ...
      grep_sur_canal auto "(entrée standard)" stdin;
