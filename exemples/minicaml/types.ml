...
let généralisation ty =
  ...
  let rec trouve_paramètres ty =
    match valeur_de ty with
    | Variable var ->
        if var.niveau > !niveau_de_liaison & (not (List.memq var !paramètres))
        then paramètres := var :: !paramètres
    | Terme(constr, arguments) ->
        Array.iter trouve_paramètres arguments in
...
let spécialisation schéma =
  ...
  | paramètres ->
      let nouvelles_inconnues =
        List.map (fun var -> (var, nouvelle_inconnue())) paramètres in
      let rec copie ty =
        match valeur_de ty with
        | Variable var as ty ->
            begin try
...
let imprime_var var =
  ...
  with Not_found ->
    let nom =
      String.make 1
        (Char.chr(Char.code 'a' + !compteur_de_variables)) in
